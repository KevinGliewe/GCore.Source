...
namespace GCore.Source.Scripting.Nuget
{
    ...
    {
        private static Regex FrameworkRegex =
            new Regex(@"(?<name>[^\s,]+)\s*,\s*[Vv]ersion\s*=\s*v(?<major>\d+)\.(?<minor>\d+)");
        ...
        private static NuGetFramework GetCurrentFramework()
        {
            ...
            var frameworkName = Assembly
                ...
                .FrameworkName ?? ".NETCoreApp,Version=v5.0"; //throw new Exception("Can't resolve framework name");

            var match = FrameworkRegex.Match(frameworkName);

            if (!match.Success)
                throw new Exception("Invalid framework: " + frameworkName.Escape());
            ...
            frameworkName = $"{match.Groups["name"]},Version=v{major}.{minor}";
